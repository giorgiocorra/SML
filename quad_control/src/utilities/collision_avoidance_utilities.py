...
def collision_avoidance_vel_smooth(p_dot_max, W, Lambda):
	...
	for i in range(len(W)):
		proj_1, proj_2 = projection_cone(p_dot_max, W[i], Lambda[i])
		proj_1 = proj_1.dot( 1 / norm(proj_1))
		proj_2 = proj_2.dot( 1 / norm(proj_2))
		if all(W.dot(proj_1) - Lambda >= EPSILON) and (proj.dot(p_dot_max) >= max):
			p_dot_star = proj_1
		...
		for j in range(i+1,len(W)):
			inters_1, inters_2 = intersection_cones(W[i],W[j], Lambda[i], Lambda[j])
			if not (inters_1.any()):
				continue
